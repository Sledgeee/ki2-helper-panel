...
function applySortFilter(array, comparator, query) {
	...
	if (query) {
		return filter(
			array,
			_birthday =>
				_birthday.student_name.toLowerCase().indexOf(query.toLowerCase()) !== -1
		)
	}
								...
								{isLoading ? (
									...
									<TableBody>
										{filteredData
											...
											.map(row => {
												const { _id: id, student_name: name, date } = row
												const selectedBirthday = selected.indexOf(id) !== -1
